...
const Reset = () => {
  ...
  const resetPass = async (
    ...
    try {
      const response = await instance.post(`password-reset-confirm `,
        {
  ...
  const resetApi = async (api_token) => {
    try {
      const response = await instance.post(`password-reset-otp `,
        {
          api_token: localStorage.getItem("token"),
        }
      );
            ...
            <form onSubmit={handleSubmit}>
                ...
                return (
                  ...
                  >
                    <label
                      ...
                      className="passwordchange sm:w-1/2 sm:p-3"
                    >
                      {item.label}
                    </label>
                    <input
                      type="text"
                      value={passData[item.id] || ""}
                      onChange={handleChange}
                      id={item.id}
                      className="passwordInput p-2 border-2 mt-2 rounded-md w-full outline-none "
              ...
              <div className="resetButtons xs:flex gap-5">
                <div className="changePassButton mt-5" onClick={handleOtp}>
                  <button className="changePassBtn p-2 bg-green-500 text-white rounded-lg">
                    Send Otp
                  </button>
                </div>
