...
const LoginLeft = () => {
  ...
  const loginOtp= async ({otp, api_token, uid ,mobile ,password, borwserData  })=>{
    try {
      ...
      if (responseData.data.status === "success") { 
        alert("Welcome to Webwise Studio");
        localStorage.setItem("token", responseData.data.api_token);  // Store token
        localStorage.getItem("mobile", mobile);
        localStorage.getItem("password", password);
        localStorage.getItem("uid", uid);
        localStorage.getItem("borwserData", borwserData);
      }
      ...
      console.log(responseData.data.borwserData)
  ...
  const { values, handleBlur, handleChange, touched, handleSubmit, errors } =
    useFormik({
      ...
      onSubmit: (values, {resetForm} ) => {
        ...
        const {mobile, password, otp, borwserData, uid, token} = values
         callLogin({mobile, password})
         loginOtp({otp,token, uid, borwserData })
        //  alert("welcome")
      },
          ...
          <form className="loginData" onSubmit={handleSubmit}>
            ...
            <div className="loginPasswordInfo  mt-3">
              ...
              <div className="password cursor-pointer flex justify-between my-2 items-center border-2 border-gray-400 outline-none rounded-md p-3">
                ...
                <div className="lockIcon bg-blue-600 p-1 rounded-full" onClick={()=>setShowPass(!showPass)}>
                  {showPass ? (
            ...
            <div className="loginOtpInfo mt-3">
               ...
               <div className="otplogin cursor-pointer flex justify-between my-2 items-center border-2 border-gray-400 outline-none rounded-md p-3">
                 <div className="loginOtp  w-[90%] ">
                   <input placeholder="Enter OTP" className="outline-none w-full" value={values.otp} id="otp" type="password"
                   onChange={handleChange}
                   onBlur={handleBlur}/>
                 </div>
                 <div
                   className="otpIcon bg-blue-600 p-1 rounded-full">
